...
# Ітерація 18 — 2026-06-28

## Зроблено

- На початку Company Info додано блок `Реквізити для каталогів`.
- Додано компактні картки основних даних, адреси, Apple Business, NAP, соцмереж і бізнес-параметрів.
- Кожне заповнене значення копіюється окремою кнопкою; відсутні поля явно позначено.
- NAP виділено як еталон для citation building.
- Додано таблицю статусів присутності для 9 бізнес-каталогів.
- Статуси зберігаються в Supabase і синхронізуються через Realtime.
- Додано migration `20260628_create_catalog_presence.sql`.
...
`Add directory credentials operations panel`

# Ітерація 19 — 2026-06-28

## Зроблено

- Прибрано зайву картку `Статус присутності`.
- Видалено пов’язану Supabase-логіку, стилі та migration для `catalog_presence`.
- Інші картки реквізитів залишено без змін.

## Назва коміту

`Remove catalog presence status card`

# Ітерація 20 — 2026-06-28

## Зроблено

- Заповнено юридичну назву `MYTRANSFER VADYM SERHIIENKO`.
- Додано NIP `5243052548`.
- Заповнено країну, місто, регіон, поштовий індекс і адресу компанії.
- Додано Facebook-профіль `MyTransferUA.PL.EU`.
- NIP додано до швидких реквізитів, а юридичні дані — окремою карткою Company Info.

## Назва коміту

`Fill company legal address and Facebook details`

# Ітерація 21 — 2026-06-28

## Зроблено

- З нижньої бібліотеки прибрано дублікати назви, сайту, телефону, email і юридичних реквізитів.
- Прибрано дублікати бізнес-категорій та географії.
- У верхніх картках збільшено заголовки, labels, значення й copy-кнопки.
- Збережено компактне компонування реквізитів.

## Назва коміту

`Remove duplicate company blocks and improve readability`

# Ітерація 22 — 2026-06-28

## Зроблено

- У верхню секцію додано high-priority картку `Юридична інформація`.
- Додано власника, реєстраційні дані, NIP і повні реквізити транспортної ліцензії.
- Додано окрему компактну картку `Контактна особа`.
- Контактну особу доповнено підтвердженими email і телефоном компанії.
- Юридичні та контактні дані додано до структурованого JSON-профілю.
- Кожне значення копіюється незалежно одним кліком.

## Назва коміту

`Add legal and contact person credentials`

# Ітерація 23 — 2026-06-28

## Зроблено

- Перед DB-маячком додано компактне меню backup-інструментів.
- JSON backup містить формат, версію, дату, пояснення, статистику та повні дані карток.
- Backup придатний для архівування, аналізу ШІ та відновлення бази.
- Додано імпорт із перевіркою формату, статусів, полів і дублікатів ID.
- Перед restore показується confirmation із датою та кількістю карток.
- Відновлення виконується одним bulk-upsert у Supabase.
- Після restore інтерфейс оновлюється, а Realtime поширює зміни іншим браузерам.
...
`Add roadmap backup and restore tools`

# Ітерація 24 — 2026-06-28

## Зроблено

- Додано автоматичне розпізнавання URL у нотатках карток.
- Посилання з `http://`, `https://` і `www.` одразу відображаються кольоровими link-chips.
- Клік відкриває URL у новій вкладці.
- Підтримується довільна кількість посилань і видаляються дублікати.
- Функція працює у компактній картці та focus modal.

## Назва коміту

`Make note URLs clickable`
